        ...
        <div data-reveal>
            ...
            @if ($reviewCount > 0)
                <div class="mt-4 flex items-center gap-3">
                    <div class="flex">
                        @for ($i = 1; $i <= 5; $i++)
                            <svg class="h-5 w-5 {{ $i <= round($averageRating) ? 'text-amber-400' : 'text-slate-300' }}" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    <span class="text-sm font-semibold text-slate-600">{{ number_format($averageRating, 1) }} / 5 · {{ $reviewCount }} verified {{ Str::plural('review', $reviewCount) }}</span>
                </div>
            @endif

            <p class="mt-4 max-w-md text-slate-500">Every review here is from a real customer — verified with a one-time code sent to their mobile. No fakes, no bots.</p>

            <div class="mt-8 space-y-4" wire:loading.remove wire:target="verifyAndSubmit">
                @forelse ($reviews as $review)
                    <div class="rounded-3xl border border-border bg-white p-5 shadow-soft" wire:key="review-{{ $review->id }}">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="font-semibold leading-tight">{{ $review->name }}</p>
                                    <span class="inline-flex items-center gap-1 text-xs font-medium text-success">
                                        <svg class="h-3.5 w-3.5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M16.7 5.3a1 1 0 010 1.4l-7 7a1 1 0 01-1.4 0l-3-3a1 1 0 011.4-1.4l2.3 2.29 6.3-6.29a1 1 0 011.4 0z" clip-rule="evenodd"/></svg>
                                        Phone-verified
                                    </span>
                                </div>
                            </div>
                        ...
                        <p class="mt-3 text-sm leading-relaxed text-slate-600">“{{ $review->message }}”</p>
                    </div>
                @empty
                    <div class="rounded-3xl border border-dashed border-border bg-white/60 p-8 text-center text-slate-500">
        ...
        <div data-reveal>
            <div class="glass rounded-3xl p-7">
                @if (! $awaitingCode)
                    ...
                    <p class="mt-1 text-sm text-slate-500">We'll send a quick code to your mobile to verify it's really you.</p>
                    ...
                    <form wire:submit="sendCode" class="mt-5 space-y-4">
                            ...
                            <div class="mt-1 flex items-center gap-1" x-data="{ hover: 0 }">
                                @for ($i = 1; $i <= 5; $i++)
                                    <button type="button" wire:click="$set('rating', {{ $i }})"
                                        ...
                                        <svg class="h-8 w-8"
                                             :class="(hover ? {{ $i }} <= hover : {{ $i }} <= {{ $rating }}) ? 'text-amber-400' : 'text-slate-300'"
                        ...
                        <button class="btn-primary w-full justify-center" wire:loading.attr="disabled" wire:target="sendCode">
                ...
                @else
                    ...
                    <p class="mt-1 text-sm text-slate-500">Enter the 6-digit code we sent to <span class="font-semibold text-slate-700">{{ $mobile }}</span>.</p>
                    ...
                    <form wire:submit="verifyAndSubmit" class="mt-5 space-y-4">
                        ...
                        <button class="btn-primary w-full justify-center" wire:loading.attr="disabled" wire:target="verifyAndSubmit">
                            ...
                            <span wire:loading wire:target="verifyAndSubmit" class="flex items-center gap-2">
                                <span class="h-4 w-4 animate-spin rounded-full border-2 border-white/40 border-t-white"></span> Verifying…
                            </span>
                        </button>

                        <div class="flex items-center justify-between text-sm">
                            <button type="button" wire:click="editDetails" class="font-medium text-slate-500 hover:text-primary">← Edit details</button>
                            <button type="button" wire:click="sendCode" class="font-medium text-primary hover:underline"
                                    wire:loading.attr="disabled" wire:target="sendCode">Resend code</button>
                        </div>
                    </form>
                @endif
            </div>
