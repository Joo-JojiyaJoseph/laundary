<div>
    ...
    <div class="glass rounded-3xl p-5" data-reveal>
        <div class="flex flex-wrap gap-3">
            <input type="search" wire:model.live.debounce.300ms="search" placeholder="Order no, customer, mobile…" class="input max-w-xs">
            <select wire:model.live="statusFilter" class="input max-w-48">
                <option value="">All statuses</option>
                @foreach ($statuses as $status)
                    <option value="{{ $status->value }}">{{ $status->label() }}</option>
                @endforeach
            </select>
            <select wire:model.live="paymentFilter" class="input max-w-40">
